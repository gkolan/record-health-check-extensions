...
## B4 — Publication completion complexity and explicit test principal

Publication completion now sends the prior-active-version collection through the system-mode update
path even when it is empty. Salesforce treats the empty collection as a no-op, preserving the
existing behavior while removing redundant control flow. The component-failure summary test now
runs under the same explicit principal convention as the rest of the class.

The full-package Recommended scan now reports zero findings. Local verification on September 21
passed 33/33 Jest tests, with 88.09% statement, 66.79% branch, 94% function, and 88.8% line
coverage, dependency audit, and Metadata API conversion. The focused Apex run could not start
because no default or explicitly authorized target org is configured.
