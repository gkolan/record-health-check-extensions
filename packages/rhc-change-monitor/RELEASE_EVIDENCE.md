...
- Eight-package repository project pinned to API `66.0` and core `2.0.4-2`.
...
| Effective principal | Trigger, handler, Queueable, core permission, sharing, CRUD, and FLS positive/negative matrix | Open |
| Limits and amplification | Measured event batch shapes, 10-policy amplification, queue backlog, and gap/overflow behavior | Open |
...
| Check | Result |
| --- | --- |
| Repository validation | Passed after final shared-org evidence update: 8 packages, 179 Markdown files, 634 local links; API 66.0 and core 2.0.4-2 contract |
...
identity, effective CDC runtime principal and least-privilege CRUD/FLS/sharing, measured event
amplification and backlog behavior, namespaced adapter packaging, or package lifecycle. The target
