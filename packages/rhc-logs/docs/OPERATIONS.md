...
the rejected payload. Retry exhaustion, subscriber state, and delivery behavior must be verified in
an org before release.

Eligibility is strictly Occurred At < now - Retention Days. A record exactly on the boundary remains.
Each run deletes only the oldest eligible RHC Diagnostic Logs up to Cleanup Batch Size. Partial
failures remain for retry. Active overlap locks skip; locks older than 60 minutes are recoverable.
